...
//  NOTA: expo-face-detector no está soportado en
//  Expo Go. Se usa simulación en todos los entornos.
//  Para detección real usar development build.
// ─────────────────────────────────────────────
...
type CameraState = 'idle' | 'scanning' | 'detected' | 'captured';
...
export default function TeenagerRegistrationScreen() {
  ...
  // Simula detección de rostro después de 2 segundos
  ...
  const renderCameraArea = () => {
    if (captured) {
      return (
        <View style={s.capturedState}>
          ...
          <Text style={[s.capturedText, { color: theme.primary }]}>
            {t('facialReg.captured')}
          </Text>
          <Text style={[s.capturedHint, { color: muted }]}>
            {t('facialReg.capturedHint')}
          </Text>
        </View>
      );
    }
    ...
    if (cameraState === 'scanning') {
      return (
        <View style={s.cameraState}>
          ...
          <Text style={[s.cameraHint, { color: muted, marginTop: 14 }]}>
            {t('facialReg.simulating')}
          </Text>
        </View>
      );
    }
      ...
      return (
        <View style={s.cameraState}>
          <View style={[s.faceFrame, { borderColor: theme.primary }]}>
            <Ionicons name="happy-outline" size={64} color={theme.primary} />
          </View>
          <View style={[s.countdownBadge, { backgroundColor: theme.primary }]}>
            <Text style={s.countdownText}>{countdown}</Text>
          </View>
          <Text style={[s.cameraHint, { color: theme.primary, fontWeight: '700' }]}>
            {t('facialReg.faceFound')}
          </Text>
        </View>
      );
    }
    ...
    return (
      <View style={s.cameraState}>
        <View style={[s.faceFrame, { borderColor: isDark ? 'rgba(101,179,97,0.40)' : 'rgba(101,179,97,0.50)' }]}>
          <Ionicons name="person-outline" size={64} color={isDark ? 'rgba(101,179,97,0.35)' : 'rgba(101,179,97,0.45)'} />
        </View>
        <Text style={[s.cameraHint, { color: muted }]}>
      ...
      <SafeAreaView style={s.safe}>
        <ScrollView contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
          <View style={[s.card, { backgroundColor: cardBg, borderColor: cardBorder }]}>
            ...
            {/* Área de cámara */}
            <TouchableOpacity
              activeOpacity={cameraState === 'idle' ? 0.8 : 1}
              onPress={cameraState === 'idle' ? handleOpenCamera : undefined}
              style={[s.cameraBox, {
                backgroundColor: isDark ? 'rgba(255,255,255,0.04)' : 'rgba(101,179,97,0.05)',
...
const s = StyleSheet.create({
  ...
  countdownBadge: { width: 52, height: 52, borderRadius: 26, alignItems: 'center', justifyContent: 'center', marginTop: 8 },
